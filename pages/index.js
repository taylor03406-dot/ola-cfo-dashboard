...
export default function Dashboard() {
  ...
  const s = {
    ...
    catName: { fontSize: 12, color: '#8ab4d0', width: 80, flexShrink: 0 },
            ...
            <div style={s.voicePanel}>
              <div style={s.voiceHeader}>
                ...
                <div>
                  ...
                  <div style={s.voiceMeta}>AI generated · {today} · 2:14</div>
                </div>
              </div>
              ...
              <div style={s.voiceFooter}>
                <span style={s.timeTxt}>{fmt(elapsed)} / {fmt(TOTAL_SECS)}</span>
                <button style={s.txBtn} onClick={() => setTxOpen(o => !o)}>{txOpen ? 'Hide transcript' : 'Read transcript'}</button>
              </div>
              {txOpen && (
                <div style={s.txBody}>
                  "Good morning. Your expense summary for Ola Thai Tapas Bar. Total spend to date is ฿{data.totalSpend.toLocaleString()} across {data.totalReceipts} receipts{data.voiceCount > 0 ? `, of which ${data.voiceCount} were logged via voice note` : ''}. Your biggest category is {data.topCategory.name} at ฿{data.topCategory.amount.toLocaleString()} — {data.topCategory.pct}% of all spend.{data.categories.length > 1 ? ` Other categories: ${data.categories.slice(1).map(c=>`${c.name} ฿${c.amount.toLocaleString()}`).join(', ')}.` : ''} Use the CFO agent below to dig into anything specific."
                </div>
              )}
            </div>
